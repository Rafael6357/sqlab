...
- `core/session_loader.py` (~280 líneas): valida JSON/CSV/XLSX/XLS + formato IA (`_normalize_ia_format`, `default_query`). Usar grep por `load_file` / `load_tablas_folder` (alias `load_csv_folder`).
- `ui/main_window.py` (~1370 líneas): ventana principal SQLab (HUD/logo/mission/matrix/consola, `work_splitter` editor|matriz, dialogs custom `_show_custom_dialog`). Buscar por nombre de widget antes de leer completo.
- `tests/` (142 tests): `test_error_friendly.py` (12), `test_sqlite_engine.py` (16), `test_session_loader.py` (19), `test_e2e_smoke.py` (34), `test_cronometro.py` (20), `test_icono.py` (3), `test_fix_matriz.py` (6), `test_carga_tablas.py` (15), `test_ejemplos_empaquetados.py` (7).
